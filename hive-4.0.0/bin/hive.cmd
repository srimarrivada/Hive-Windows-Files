...
set USE_BEELINE_FOR_HIVE_CLI=true
if [%SERVICE%] == [cli] ( 
  if [%USE_BEELINE_FOR_HIVE_CLI%] == [true] (
    set CLIUSER=%USERNAME%
    set SERVICE=beeline
  )
)
...
@rem pass aux path to classpath
if defined AUX_CLASSPATH set CLASSPATH=%CLASSPATH%;%AUX_CLASSPATH%
...
:AddServices
    for /f %%a in ('dir /b %HIVE_BIN_PATH%\ext\*.cmd') do (
		set SERVICE_LIST=!SERVICE_LIST! %%~na
	)	
